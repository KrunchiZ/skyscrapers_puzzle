...
/*   Updated: 2026/04/22 03:50:29 by kchiang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pre_solve(int *answer, t_var *var)
{
	...
	{
		if (var->clue[0][i] == 1)
			answer[i] = var->row_size;
		if (var->clue[1][i] == 1)
			answer[var->ans_size - var->row_size + i] = var->row_size;
		if (var->clue[2][i] == 1)
			answer[i * var->row_size] = var->row_size;
		if (var->clue[3][i] == 1)
			answer[i * var->row_size + var->row_size - 1] = var->row_size;
	}
}
